...
class ArbitrationConfig(BaseModel):
    ...
    consensus_samples: int = Field(1, description="Number of augmented re-reads per crop (1=deterministic, >1=multi-sample)")
    consensus_use_sampling_variant: bool = Field(False, description="Use one temperature-sampled variant among the re-reads")
    consensus_temperature: float = Field(0.0)
