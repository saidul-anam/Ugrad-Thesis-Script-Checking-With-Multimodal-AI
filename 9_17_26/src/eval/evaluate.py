...
def _base_meta(cfg: EvalConfig, row: dict, bp: BuiltPrompt, model: str, run: int) -> dict:
    return {
        "task_id": row["task_id"],
        "exam_set": row.get("exam_set") or cfg.exam_set,
...
def _is_synthesised_for(path: Path, row: dict, model: str, run: int) -> bool:
    """True if `path` already holds the synthesised record for exactly this unit."""
    try:
        meta = json.loads(path.read_text(encoding="utf-8")).get("_metadata", {})
    except (OSError, json.JSONDecodeError):
        return False
    return bool(
        meta.get("synthesised")
        and meta.get("script_id") == row["script_id"]
        and str(meta.get("question_no")) == str(row["question_no"])
        and meta.get("model") == model
        and int(meta.get("run_index", -1)) == run
    )
...
def evaluate_one(
    ...
    if row.get("extraction_status") == "not_attempted":
        # Reuse only a record that is itself a synthesised one for THIS answer.
        # task_id is positional (PARA_001 is "the first paragraph row"), so
        # adding a script shifts what a given id refers to; a bare exists()
        # check would then leave a real graded record standing where a Band 0
        # belongs. Cheap to re-synthesise, so verify rather than assume.
        if not force and out.exists() and _is_synthesised_for(out, row, model, run):
    ...
    if not force and out.exists():
        try:
            ...
            pm = prev.get("_metadata", {})
            # cache_key alone would be enough for the SCORE (identical prompt ->
            # identical grading), but the record also carries script_id, so
            # confirm it is this script's before reusing the file wholesale.
            if pm.get("cache_key") == ck and pm.get("script_id") == row["script_id"]:
