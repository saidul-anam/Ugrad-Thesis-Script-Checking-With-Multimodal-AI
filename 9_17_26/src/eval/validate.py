...
def _bands_for_total(total: float, max_mark: int) -> set[str]:
    """Bands a total may legitimately be reported in.

    The rubric's band table has integer boundaries, but gemma awards half-marks
    — and a half-mark can land between two bands: at MAX_MARK 5, Band 3 is
    exactly 3 and Band 4 is 4-5, so 3.5 belongs to neither. Rounding to pick a
    winner would fail a grader for a choice the rubric never specified (and
    Python's banker's rounding sends 3.5 UP to 4, which is the less defensible
    of the two). So a fractional total accepts the band on either side; an
    integer total still has exactly one right answer.
    """
    import math

    if float(total).is_integer():
        return {band_for(int(total), max_mark)}
    return {
        b for b in (band_for(math.floor(total), max_mark),
                    band_for(math.ceil(total), max_mark))
        if b is not None
    }
...
def run_assertions(records: list[dict], extraction: dict[str, dict]) -> list[Check]:
    ...
    item_ok = True          # item-scored rows: only context_content_data, never capped
    unknown_tasks: set[tuple[str, int]] = set()
    for r in records:
        m = r["_metadata"]
        qtype = r["task_type"]
        raw = r["raw_subscores"]
        ...
        if abs(raw_sum - float(raw["raw_total"])) > TOL:
            sum_ok = False
        ceilings = ceilings_for(qtype, max_mark)
        if not ceilings:
            unknown_tasks.add((qtype, max_mark))
        for c in FOUR_CRITERIA:
            cap = ceilings.get(c, 99)
            if float(raw[c]) > cap + TOL or float(sb[c]) > cap + TOL:
                ceil_ok = False
        total = float(sb["total_score"])
        cap_reason = r["structural_audit"]["applied_cap_reason"]
        cap_val = cap_value(cap_reason, max_mark)
        limit = min(max_mark, cap_val) if cap_val is not None else max_mark
        if qtype in ITEM_SCORED_TYPES and not m.get("synthesised"):
            others = ("structure_format_brevity", "language_mechanics",
                      "originality_comparisons_paraphrase")
            if any(abs(float(sb[c])) > TOL for c in others) or cap_reason != "None":
                item_ok = False
        if total > limit + TOL:
            total_ok = False
        if r["performance_band"] not in _bands_for_total(total, max_mark):
            band_ok = False
    ...
    _add(checks, not unknown_tasks,
         "every (question_type, max_mark) has a ceiling row"
         + (f"; UNKNOWN {sorted(unknown_tasks)}" if unknown_tasks else ""))
    ...
    # global-identical fields (one exam set per output directory, so one rubric)
    sets = {r["_metadata"].get("exam_set", "") for r in records}
    rubric_hashes = {r["_metadata"]["rubric_hash"] for r in records}
    think = {r["_metadata"]["thinking_level"] for r in records}
    _add(checks, len(sets) == 1, f"all records come from one exam set ({sorted(sets)})")
...
def validate_all(cfg: EvalConfig) -> ValidationOutput:
    ...
    report_path = cfg.paths.logs / f"evaluation_report_{cfg.exam_set}.md"
