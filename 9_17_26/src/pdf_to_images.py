...
def discover_scripts(cfg: Config, exam_set: str | None = None) -> list[tuple[str, Path]]:
    """All PDFs under the raw_pdfs dir, sorted by script id, as (script_id, path).

    The search is recursive: exam sets are filed in per-set subdirectories
    (data/raw_pdfs/SE_07_Q1/...), but a script's set comes from its id, not its
    directory, so a loose PDF at the top level still works. `exam_set` keeps
    only the scripts belonging to that set.
    """
    pdfs = sorted(cfg.paths.raw_pdfs.rglob("*.pdf"), key=lambda p: p.stem)
    found = [(script_id_from_path(p), p) for p in pdfs]
    if exam_set:
        found = [(sid, p) for sid, p in found if exam_set_of(sid) == exam_set]
    return found
