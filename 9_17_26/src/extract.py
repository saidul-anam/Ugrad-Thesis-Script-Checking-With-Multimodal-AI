...
coherent. The system prompt is the full contents of the prompt file configured
for the script's exam set (prompts/extraction_prompt_v2.md for Class XI,
prompts/extraction_prompt_c7_v1.md for Class VII), loaded verbatim from disk at
runtime (never inlined or paraphrased). Model output is fence-stripped, JSON-parsed (one retry
...
from src.config import Config, exam_set_of
...
# ---------------------------------------------------------------------------
# Pydantic models — mirror the extraction prompts' OUTPUT FORMAT exactly (both
# prompt files emit the same schema).
...
# Helpers
# ---------------------------------------------------------------------------
def prompt_path_for(cfg: Config, script_id: str) -> Path:
    """The extraction prompt for this script's exam set.

    Each question paper needs its own prompt (a Class VII script carries MCQ
    grids and tables a Class XI prompt says nothing about), so the prompt is a
    property of the set. `paths.prompt_file` remains only as a fallback for a
    script whose set is not configured.
    """
    try:
        return cfg.exam_set_for_script(script_id).prompt_file
    except KeyError:
        return cfg.paths.prompt_file


def load_prompt(cfg: Config, script_id: str | None = None) -> tuple[str, str, str]:
    """Return (prompt_text, prompt_sha256, prompt_version). Loaded from disk."""
    path = prompt_path_for(cfg, script_id) if script_id else cfg.paths.prompt_file
    if not path.exists():
...
def extract_script(
    ...
    *,
    ...
    started = monotonic()
    prompt_text, prompt_hash, prompt_version = load_prompt(cfg, script_id)
    ...
    metadata = {
        ...
        "exam_set": exam_set_of(script_id),
        "prompt_version": prompt_version,
        "prompt_file": prompt_path_for(cfg, script_id).name,
