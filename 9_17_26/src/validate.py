...
PARAGRAPH_BREAK_MAX = 3      # paragraph-answer blank-line count
MEAN_SEGMENT_WORDS_MIN = 15  # paragraph-answer mean words per blank-line segment
DUPLICATE_RATIO = 0.85       # difflib ratio between two transcripts

# Which questions are paragraph-type or prose-type is a property of the exam set,
# read from its task_stems.csv rather than hard-coded here. The absorbed-
# correction (isolated single word on its own line) check runs only on
# continuous-prose questions: item-scored answers, letters and dialogues all
# legitimately put a lone word on a line.
...
def evaluate_script(
    ...
    checker,
    *,
    types_by_qno: dict[str, str] | None = None,
    max_question: int = 11,
) -> ScriptFlags:
    ...
    types_by_qno = types_by_qno or {}
    target_set = {normalize_qno(q) for q in target_questions}
    prose_targets = {q for q in target_set if types_by_qno.get(q) in PROSE_TYPES}
    paragraph_targets = {q for q in target_set if types_by_qno.get(q) == "Paragraph"}
...
def _types_by_qno(es: ExamSetConfig) -> dict[str, str]:
    """{question_no: question_type} for one exam set, from its task stems."""
    from src.build_csv import load_task_stems  # local: build_csv imports this module

    return {q: m.question_type for q, m in load_task_stems(es.task_stems).items()}


def validate_all(cfg: Config, set_id: str) -> tuple[list[ScriptFlags], Path]:
    try:
    ...
    targets = [str(q) for q in es.target_questions]
    types_by_qno = _types_by_qno(es)
    transcripts = [
        d for d in _load_transcripts(cfg)
        if exam_set_of(
            d.get("_metadata", {}).get("script_id") or d.get("script_id", "")
        ) == set_id
    ]
    results = [
        evaluate_script(d, targets, checker, types_by_qno=types_by_qno,
                        max_question=es.max_question)
        for d in transcripts
    ]
    _apply_peer_error_rate(results)

    report = build_report(results, targets)
    report_path = cfg.paths.logs / f"validation_report_{set_id}.md"
