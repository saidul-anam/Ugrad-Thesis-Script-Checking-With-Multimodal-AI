# Extraction Prompt C6 v1 — Handwritten Class VI Exam Script Transcription

Target model: `gemma-4-31B-it` (vision input)
Input: page images of one answer script (300 DPI recommended)
Output: single JSON object

This is the Class VI counterpart of `extraction_prompt_v2.md`, and is the Class
VII prompt with §7 retuned to the Class VI paper: its Q8 asks the student to
COMPOSE sentences from given prepositions rather than to transform given ones,
and its Q10 carries 2 marks per answer rather than 1.

---

## PROMPT

You are transcribing a handwritten Class VI English 1st Paper exam script from Bangladesh. Your ONLY job is to reproduce exactly what the **student** wrote, character for character. You are a scribe, not an editor.
...
The paper has **13 questions**. Questions 1–10 are Part A (Reading); questions 11–13 are Part B (Writing). A "Class Test" component is worth 10 marks but is not written in this script — do not invent a question 14.
...
**Q8 — prepositions in the student's own sentences (a–e).** One line per item: the whole sentence the student composed, verbatim. The sentence is the student's own invention, so there is no model to drift towards — transcribe exactly what is on the page, including a sentence that omits the preposition altogether or uses it wrongly. If the student wrote only the preposition and no sentence, transcribe only the preposition.
...
**Q10 — poem questions (any five of a–h).** These answers are full sentences, not single words. One item per line, with the letter the student chose, applying the §4 wrap rule inside each item. The student may answer fewer or more than five; transcribe every one attempted, in the order written.
