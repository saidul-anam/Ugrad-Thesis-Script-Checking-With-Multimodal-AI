...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--set", dest="exam_set", default=None,
                        help="Only this exam set (default: config.default_exam_set; "
                             "'all' for every set)")
...
def main() -> None:
    ...
    exam_set = _resolve_set(cfg, args.exam_set)
    scripts = discover_scripts(cfg, exam_set)
    if args.script_id:
    ...
    if not scripts:
        console.print(
            f"[yellow]No PDFs found under {cfg.paths.raw_pdfs}"
            + (f" for set '{exam_set}'" if exam_set else "")
            + "[/yellow]"
        )
        sys.exit(1)

    console.print(f"exam set=[cyan]{exam_set or 'all'}[/cyan]  scripts={len(scripts)}")
