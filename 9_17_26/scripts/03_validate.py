...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--set", dest="exam_set", default=None,
                        help="Exam set to validate (default: config.default_exam_set)")
...
def main() -> None:
    ...
    results, report_path = validate_all(cfg, set_id)
    if not results:
        console.print(
            f"[yellow]No {set_id} transcripts found in {cfg.paths.transcripts}[/yellow]"
        )
        sys.exit(1)

    table = Table(title=f"Stage 4 - validation ({set_id})")
