...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--set", dest="exam_set", default=None,
                   help="Exam set to grade (default: config.default_exam_set)")
    p.add_argument("--model", choices=["gemini", "gemma", "both"], default=None,
                   help="Grader(s) to run (default: evaluation.default_models)")
...
def main() -> None:
    ...
    cfg = load_eval_config(args.config, args.exam_set)
    cfg.ensure_dirs()
    ...
    console.print(
        f"set={cfg.exam_set} rows={len(rows)} models={models} k={k} "
    ...
    table = Table(title=f"Stage 2 - evaluation ({cfg.exam_set})")
