...
Grain: **one row per (script × question)**. Which questions are in scope is a property of the
exam set (see `docs/exam_sets.md`): Class XI `SE_11_Q1` covers 3, 7, 8, 9, 10, 11; Class VII
`SE_07_Q1` covers all 13.
Expected row count is `scripts × len(target_questions)` for the set, asserted at build time.
...
|---|---|---|---|
| `task_id` | string | `CHART_001` | **Primary key within an exam set.** `{TYPE}_{NNN}`, zero-padded. Class XI prefixes: `SUMMARY`, `PARA`, `CHART`, `STORY`, `LETTER`, `THEME`. Class VII adds `MCQ`, `SHORT`, `GAP`, `VOCAB`, `TABLE`, `TF`, `TRANS`, `REARR`, `POEM`, `DIALOG` |
| `exam_set` | string | `SE_07_Q1` | Which question paper this row belongs to; the script id minus its serial |
| `script_id` | string | `SE_11_Q1_0001` | PDF filename stem |
| `question_no` | string | `8` | **String, not int** — `1A`/`1B` exist if scope widens |
| `question_type` | enum | `Graph_Chart` | Class XI: `Summary`\|`Paragraph`\|`Graph_Chart`\|`Story`\|`Letter_Email`\|`Theme`. Class VII adds `MCQ`\|`Short_Answer`\|`Gap_Fill`\|`Vocabulary`\|`Table_Completion`\|`True_False`\|`Sentence_Transformation`\|`Rearrange`\|`Poem_Short_Answer`\|`Dialogue`. Must match `task_type` in the set's rubric |
| `max_mark` | int | `10` | From the set's task stems (Class XI 3→10, 7→10, 8→10, 9→7, 10→5, 11→8). Carried so the grader never re-derives it |
